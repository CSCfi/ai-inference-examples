...
While the job is running, you can connect to the vLLM server from a process running on the same node.
For example, the following opens a terminal on the node running vLLM and sends a request via the cURL command line tool:

On LUMI with DeepSeek-R1-Distill-Qwen-32B (change `"model"` to `deepseek-ai/DeepSeek-R1-0528` if you're running `run-vllm-lumi16.sh`)
...
```
**Note:** The script uses DeepSeek-R1-Distill-Qwen-32B by default (change `model` to `Qwen/Qwen3-32B` for Roihu, or to `deepseek-ai/DeepSeek-R1-0528` if you're running `run-vllm-lumi16.sh`)
...
```bash
sbatch run-ollama-lumi8.sh
```

## TODO
- [ ] the Ollama script doesn't seem to use all GPUs, probably the script is reserving too much
